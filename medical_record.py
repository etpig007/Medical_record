...
tabControl.add(tab1, text = 'NE')
tabControl.add(tab2, text ='Dermatome')
...
def insertTEXT():
    ...
    txt.insert('end', 'Dermatome: ' + Derma.get() + '\n')
...
# NE
General = LabelFrame(tab1, text='NE', labelanchor='n',width=500)
...
Entry(General, width=23, textvariable=EOM).grid(row=3, column=1,columnspan=3,pady=1)
Label(General, text='Muscle power: ').grid(row=4)
Entry(General, width=23, textvariable=MP).grid(row=4, column=1,columnspan=3,pady=1)
Label(General, text='Dermatome: ').grid(row=5)
Entry(General, width=23, textvariable=Derma).grid(row=5, column=1,columnspan=3,pady=1)
Label(General, text='DTR: ').grid(row=6)
Entry(General, width=23, textvariable=DTR).grid(row=6, column=1,columnspan=3,pady=1)
Label(General, text='Visual Field: ').grid(row=7)
Radiobutton(General, text='Normal',variable=VisualField, value='Normal').grid(row=7,column=1)
Radiobutton(General, text='Abnormal',variable=VisualField, value='Abnormal').grid(row=7,column=2)
Label(General, text='FingerNFinger ').grid(row=8)
Radiobutton(General, text='Normal',variable=FingerNFinger, value='Normal').grid(row=8,column=1)
Radiobutton(General, text='Abnormal',variable=FingerNFinger, value='Abnormal').grid(row=8,column=2)
Label(General, text='Tendom Gait ').grid(row=9)
Radiobutton(General, text='Normal',variable=TendomGait, value='Normal').grid(row=9,column=1)
Radiobutton(General, text='Abnormal',variable=TendomGait, value='Abnormal').grid(row=9,column=2)
General.pack(padx=20, pady=10, anchor='w')

# Cervical PE
Cervical = LabelFrame(tab1, text='C spine', labelanchor='n')
...
# Lumbar PE
Lumbar = LabelFrame(tab1, text='Lumbar', labelanchor='n')
...
# Carpal tunnel
Carpal = LabelFrame(tab1, text='Capal tunnel examiniations', labelanchor='n')
...
#display 
new_img = Image.open('dermatome.jpg').resize((450,650))
img = ImageTk.PhotoImage(new_img)
panel = Label(tab2, image = img)
panel.pack()

# check button
txt = Text(tab1, width=70, height=45)
txt.place(x=350, y=10)

btn1 = Button(tab1, text='確定', command=insertTEXT)
btn1.place(x=20, y=600)
btn2 = Button(tab1, text='複製文字', command=copyTEXT)
